...
const ReceiptRegister = () => {
  ...
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      ...
      return vNo.includes(q) || cust.includes(q) || mode.includes(q);
    });
  }, [data, searchQuery]);

  const totalReceipts = filteredData.reduce((acc, i) => acc + (i.amount || 0), 0);
  const bankReceipts = filteredData.filter((i) => i.mode !== "cash").reduce((acc, i) => acc + (i.amount || 0), 0);
  const cashReceipts = filteredData.filter((i) => i.mode === "cash").reduce((acc, i) => acc + (i.amount || 0), 0);
  ...
  const exportToCSV = () => {
    ...
    const headers = ["Voucher No", "Date", "Customer Name", "Payment Mode", "Bank / Ref No", "Amount"];
    ...
    link.setAttribute("download", `Receipt_Register_${new Date().toISOString().slice(0, 10)}.csv`);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }} className="pb-10 relative">
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }} className="md:!flex-row md:!items-center md:!justify-between">
        <div>
          ...
          <p style={{ fontSize: 13, color: "#6b7280", marginTop: 4, margin: 0 }}>
            Audit log of cash collections, bank deposits, and customer receivables cleared.
          </p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 14 }}>
        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, padding: "16px 20px", border: "1px solid #e5e7eb" }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 }}>Total Receipt Vouchers</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: "#111827" }}>{filteredData.length} Vouchers</div>
          <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 2 }}>Count of collection entries</div>
        </div>

        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, padding: "16px 20px", border: "1px solid #e5e7eb" }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 }}>Bank Deposits</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: "#2563eb" }}>{formatCurrency(bankReceipts)}</div>
          <div style={{ fontSize: 11, color: "#2563eb", marginTop: 2 }}>NEFT / RTGS / UPI / Cheque</div>
        </div>

        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, padding: "16px 20px", border: "1px solid #e5e7eb" }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 }}>Cash Collections</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: "#d97706" }}>{formatCurrency(cashReceipts)}</div>
          <div style={{ fontSize: 11, color: "#d97706", marginTop: 2 }}>Direct cash receipts</div>
        </div>
        ...
        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, padding: "16px 20px", border: "1px solid #e5e7eb", background: "linear-gradient(135deg, #111113 0%, #1f1f23 100%)", color: "#ffffff" }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 }}>Total Collection Amount</div>
          <div style={{ fontSize: 22, fontWeight: 800, color: "#16a34a" }}>{formatCurrency(totalReceipts)}</div>
          <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 2 }}>Total customer receipts</div>
        </div>
      </div>
      ...
      <div style={{ backgroundColor: "#ffffff", borderRadius: 16, padding: "14px 18px", border: "1px solid #e5e7eb", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <form onSubmit={handleSearch} style={{ display: "flex", alignItems: "center", gap: 12, flex: 1, flexWrap: "wrap" }}>
          <div style={{ position: "relative", flex: 1, minWidth: 240, maxWidth: 380 }}>
            ...
            <input
              type="text"
              placeholder="Search voucher, customer, mode..."
              value={searchQuery}
      ...
      {loading ? (
        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, border: "1px solid #e5e7eb", padding: 60, textAlign: "center" }}>
          <div style={{ width: 52, height: 52, borderRadius: 14, backgroundColor: "rgba(253,75,35,0.08)", color: "#FD4B23", margin: "0 auto 16px auto", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Receipt size={24} className="animate-spin" />
          </div>
          <h3 style={{ fontSize: 16, fontWeight: 700, color: "#111827", margin: 0 }}>Loading Receipt Register...</h3>
          <p style={{ fontSize: 13, color: "#9ca3af", marginTop: 4 }}>Retrieving collection vouchers</p>
        </div>
        ...
        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, border: "1px solid #e5e7eb", padding: 60, textAlign: "center" }}>
          ...
          <h3 style={{ fontSize: 16, fontWeight: 700, color: "#111827", margin: 0 }}>No Receipt Records Found</h3>
          <p style={{ fontSize: 13, color: "#6b7280", maxWidth: 360, margin: "6px auto 20px auto", lineHeight: 1.5 }}>
            No receipt vouchers matched your search query or date range filters.
          </p>
        </div>
      ) : (
