...
const PurchaseRegister = () => {
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-gray-800 mb-6">Purchase Register</h2>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              <th className="py-3 px-4 font-semibold text-sm text-gray-700">Purchase No</th>
              ...
              <th className="py-3 px-4 font-semibold text-sm text-gray-700">Subtotal</th>
              <th className="py-3 px-4 font-semibold text-sm text-gray-700">CGST</th>
              <th className="py-3 px-4 font-semibold text-sm text-gray-700">SGST</th>
            ...
            {loading ? (
              ...
              data.map((item) => (
                <tr key={item._id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="py-3 px-4 font-medium text-gray-800">{item.purchaseNumber}</td>
                  <td className="py-3 px-4 text-gray-600">{formatDate(item.date)}</td>
                  <td className="py-3 px-4 text-gray-600">{item.supplierId?.name || "N/A"}</td>
                  <td className="py-3 px-4 text-gray-600">₹{(item.subtotal || 0).toLocaleString("en-IN")}</td>
                  <td className="py-3 px-4 text-gray-600">₹{(item.cgst || 0).toLocaleString("en-IN")}</td>
                  <td className="py-3 px-4 text-gray-600">₹{(item.sgst || 0).toLocaleString("en-IN")}</td>
                  <td className="py-3 px-4 font-semibold text-gray-800">
                    ₹{(item.totalAmount || 0).toLocaleString("en-IN")}
                  </td>
                  <td className="py-3 px-4">
                    <span className="inline-block px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-700 capitalize">
                      {item.status || "pending"}
                    </span>
                  </td>
                </tr>
