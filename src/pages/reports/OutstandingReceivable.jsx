...
const OutstandingReceivable = () => {
  ...
  const totalBilled = filteredData.reduce((acc, i) => acc + (i.totalSales || 0), 0);
  const totalCollected = filteredData.reduce((acc, i) => acc + (i.totalReceived || 0), 0);
  ...
  const exportToCSV = () => {
    ...
    const headers = ["Customer Name", "Total Sales (₹)", "Total Collected (₹)", "Outstanding Due (₹)"];
    ...
    link.setAttribute("download", `Outstanding_Receivable_${new Date().toISOString().slice(0, 10)}.csv`);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }} className="pb-10 relative">
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }} className="md:!flex-row md:!items-center md:!justify-between">
        <div>
          ...
          <p style={{ fontSize: 13, color: "#6b7280", marginTop: 4, margin: 0 }}>
            Customer receivables summary, pending sales balances, and collection status.
          </p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 14 }}>
        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, padding: "16px 20px", border: "1px solid #e5e7eb" }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 }}>Clients Count</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: "#111827" }}>{filteredData.length} Clients</div>
          <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 2 }}>With sales history</div>
        </div>

        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, padding: "16px 20px", border: "1px solid #e5e7eb" }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 }}>Total Sales Invoiced</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: "#111827" }}>{formatCurrency(totalBilled)}</div>
          <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 2 }}>Gross billed sales</div>
        </div>

        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, padding: "16px 20px", border: "1px solid #e5e7eb" }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 }}>Total Collected</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: "#16a34a" }}>{formatCurrency(totalCollected)}</div>
          <div style={{ fontSize: 11, color: "#16a34a", marginTop: 2 }}>Cleared receipts total</div>
        </div>
        ...
        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, padding: "16px 20px", border: "1px solid #e5e7eb", background: "linear-gradient(135deg, #111113 0%, #1f1f23 100%)", color: "#ffffff" }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 800, color: "#FD4B23" }}>{formatCurrency(totalReceivable)}</div>
          <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 2 }}>Net receivable balance</div>
        </div>
      </div>
      ...
      {loading ? (
        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, border: "1px solid #e5e7eb", padding: 60, textAlign: "center" }}>
          ...
          <h3 style={{ fontSize: 16, fontWeight: 700, color: "#111827", margin: 0 }}>Loading Outstanding Receivables...</h3>
          <p style={{ fontSize: 13, color: "#9ca3af", marginTop: 4 }}>Calculating client balances</p>
        </div>
        ...
        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, border: "1px solid #e5e7eb", padding: 60, textAlign: "center" }}>
          ...
          <h3 style={{ fontSize: 16, fontWeight: 700, color: "#111827", margin: 0 }}>No Outstanding Receivables</h3>
          <p style={{ fontSize: 13, color: "#6b7280", maxWidth: 360, margin: "6px auto 20px auto", lineHeight: 1.5 }}>
            No customer receivables found matching your search query. All accounts are settled!
          </p>
        </div>
      ) : (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
              <thead>
                <tr style={{ backgroundColor: "#f9fafb", borderBottom: "1px solid #e5e7eb", fontSize: 11, fontWeight: 700, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                  <th style={{ padding: "14px 20px" }}>Customer / Client Name</th>
                  <th style={{ padding: "14px 20px", textAlign: "right" }}>Total Sales (₹)</th>
                  <th style={{ padding: "14px 20px", textAlign: "right" }}>Total Collected (₹)</th>
                  ...
                  >
                    ...
                    <td style={{ padding: "14px 20px", textAlign: "center" }}>
                      <span style={{ fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 20, backgroundColor: item.outstanding <= 0 ? "#f0fdf4" : "#fef2f2", color: item.outstanding <= 0 ? "#16a34a" : "#dc2626", border: item.outstanding <= 0 ? "1px solid #bbf7d0" : "1px solid #fecaca" }}>
                        {item.outstanding <= 0 ? "Settled" : "Pending"}
                      </span>
                    </td>
                  </tr>
                ))}
