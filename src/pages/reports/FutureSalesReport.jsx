...
const FutureSalesReport = () => {
  ...
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      ...
      return oNo.includes(q) || cust.includes(q);
    });
  }, [data, searchQuery]);
  ...
  const exportToCSV = () => {
    ...
    const headers = ["Order Number", "Order Date", "Expected Date", "Customer Name", "Subtotal", "GST", "Total Amount", "Status"];
    ...
    link.setAttribute("download", `Future_Sales_Report_${new Date().toISOString().slice(0, 10)}.csv`);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }} className="pb-10 relative">
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }} className="md:!flex-row md:!items-center md:!justify-between">
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: "#111827", letterSpacing: "-0.02em", margin: 0 }}>
            Future Sales Order Register
          </h1>
          <p style={{ fontSize: 13, color: "#6b7280", marginTop: 4, margin: 0 }}>
            Log of advance client sales orders, installation target dates, and projected revenues.
          </p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 14 }}>
        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, padding: "16px 20px", border: "1px solid #e5e7eb" }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 }}>Total Advance Orders</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: "#111827" }}>{filteredData.length} Orders</div>
          <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 2 }}>Booked sales order count</div>
        </div>
        ...
        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, padding: "16px 20px", border: "1px solid #e5e7eb" }}>
          ...
          <div style={{ fontSize: 11, color: "#d97706", marginTop: 2 }}>Projected tax collection</div>
        </div>
        ...
        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, padding: "16px 20px", border: "1px solid #e5e7eb", background: "linear-gradient(135deg, #111113 0%, #1f1f23 100%)", color: "#ffffff" }}>
          ...
          <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 2 }}>Projected sales total</div>
        </div>
      </div>
      ...
      {loading ? (
        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, border: "1px solid #e5e7eb", padding: 60, textAlign: "center" }}>
          ...
          <h3 style={{ fontSize: 16, fontWeight: 700, color: "#111827", margin: 0 }}>Loading Future Sales...</h3>
        ...
        <div style={{ backgroundColor: "#ffffff", borderRadius: 16, border: "1px solid #e5e7eb", padding: 60, textAlign: "center" }}>
          ...
          <h3 style={{ fontSize: 16, fontWeight: 700, color: "#111827", margin: 0 }}>No Future Sales Records</h3>
          <p style={{ fontSize: 13, color: "#6b7280", maxWidth: 360, margin: "6px auto 20px auto", lineHeight: 1.5 }}>
            No future sales bookings matched your search query or date range filters.
          </p>
        </div>
      ) : (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
              <thead>
                <tr style={{ backgroundColor: "#f9fafb", borderBottom: "1px solid #e5e7eb", fontSize: 11, fontWeight: 700, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                  ...
                  <th style={{ padding: "14px 20px" }}>Expected Installation Date</th>
